          ...
          data.map((item) => {
            ...
            return (
                <div key={item.label} className="grid grid-cols-[80px_1fr_40px] items-center gap-3" aria-label={`${item.label}: ${item.value} albums`}>
                    <span className="text-sm font-medium text-zinc-600 text-right truncate" title={item.label}>
                        {item.label}
                    </span>
...
const TopItemsList = ({ data, title }: { data: { label: string; value: number }[], title: string }) => (
    <div className="bg-white rounded-lg border border-zinc-200 p-6">
        <h3 className="text-lg font-bold text-zinc-800 mb-4">{title}</h3>
                ...
                <ul className="divide-y divide-zinc-200">
                    {data.map(item => (
                        <li key={item.label} className="py-2 flex justify-between items-center">
                            <span className="text-zinc-700">{capitalizeWords(item.label)}</span>
...
const DashboardView: React.FC<DashboardViewProps> = ({ cds }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <BarChart data={albumsByDecade} title="Albums by Decade" />
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <TopItemsList data={topGenres} title="Top 5 Genres" />
            <TopItemsList data={topLabels} title="Top 5 Record Labels" />
        </div>
    </div>
  );
};
