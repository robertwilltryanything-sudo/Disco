  ...
  const handleConvertExistingLinks = () => {
    ...
    if (convertedCount > 0 && onUpdateCollection) {
      ...
      setBulkResultMsg(`Successfully converted ${convertedCount} existing Plex links to ${
        targetFormat === 'listen_plex' ? 'Plexamp Share format (https://listen.plex.tv/album...)' :
        targetFormat === 'plexamp_app' ? 'Plexamp App format (plexamp://...)' : 'Plex Web format'
      }!`);
    } else {
  ...
  const handleBulkAutoLink = async (onlyUnlinked = true) => {
    ...
    for (let i = 0; i < itemsToMatch.length; i++) {
      ...
      try {
        ...
        if (result && (result.bestPlexUrl || result.hostedWebUrl || result.webUrl)) {
          const matchedUrl = result.bestPlexUrl || result.hostedWebUrl || result.webUrl;
        ...
        <div className="space-y-4 text-sm">
          ...
          <div>
            ...
            <div className="space-y-2 text-xs">
              <label className="flex items-start gap-2 cursor-pointer p-2.5 rounded-lg border border-zinc-200 hover:bg-zinc-50/80 transition-colors">
                <input 
                  type="radio" 
                  name="linkFormat" 
                  checked={(config.linkFormat || 'listen_plex') === 'listen_plex'} 
                  onChange={() => setConfig(c => ({ ...c, linkFormat: 'listen_plex' }))}
                  disabled={isBulkMatching}
                  className="mt-0.5 text-amber-500 focus:ring-amber-500"
                />
                <div>
                  <span className="font-bold text-zinc-900 block">Plexamp Share Link (Recommended)</span>
                  <span className="text-[11px] text-zinc-500 block leading-tight mt-0.5">
                    <code className="bg-zinc-100 px-1 rounded text-[10px] font-mono">https://listen.plex.tv/album?ratingKey=...</code> — Same universal link generated by Plexamp's "Share → Copy Link"
                  </span>
                </div>
              </label>
              ...
              <label className="flex items-start gap-2 cursor-pointer p-2.5 rounded-lg border border-zinc-200 hover:bg-zinc-50/80 transition-colors">
                <input 
                  type="radio" 
                  name="linkFormat" 
                  checked={config.linkFormat === 'plexamp_app'} 
                  onChange={() => setConfig(c => ({ ...c, linkFormat: 'plexamp_app' }))}
                  disabled={isBulkMatching}
                  className="mt-0.5 text-amber-500 focus:ring-amber-500"
                />
                <div>
                  <span className="font-bold text-zinc-900 block">Plexamp App Protocol</span>
                  <span className="text-[11px] text-zinc-500 block leading-tight mt-0.5">
                    <code className="bg-zinc-100 px-1 rounded text-[10px] font-mono">plexamp://album?ratingKey=...</code> — Directly launches installed Plexamp desktop/mobile app
                  </span>
                </div>
              </label>
